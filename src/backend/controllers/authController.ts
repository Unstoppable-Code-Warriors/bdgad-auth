...
export const updateProfile = async (c: ValidatedContext) => {
  try {
    ...
    if (phone && phone.trim()) {
      const duplicatePhoneUser = await db
        .select({
          id: users.id,
          metadata: users.metadata,
        })
        .from(users)
        .where(and(ne(users.id, userId), eq(users.status, "active")));

      // Check if any other user has this phone number in their metadata
      const phoneExists = duplicatePhoneUser.some((user) => {
        const metadata = user.metadata as Record<string, string> | null;
        return metadata?.phone === phone.trim();
      });

      if (phoneExists) {
        return c.json(errorResponses.duplicatePhone);
      }
    }
...
export const googleOAuthCallback = async (c: Context) => {
  try {
    ...
    if (!code) {
      ...
      if (state) {
        ...
        if (stateData && Date.now() - stateData.timestamp <= 10 * 60 * 1000) {
          ...
          errorUrl.searchParams.set("message", "Authorization code is missing");
          return c.redirect(errorUrl.toString());
        }
      }
    ...
    if (!tokenResponse.ok) {
      ...
      console.error("Auth service callback URL used:", authServiceCallbackUrl);
      console.error(
        "Google Client ID:",
